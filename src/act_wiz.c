...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	victim->hunting ? victim->hunting->name : "(none)",
      ...
      pager_printf_color( ch, "&cDeity   : &w%-13s&w   &cFavor  : &w%-5d &cGlory : &w%-3d (%-3d)     &cHonour : &w%-3d\n\r",
...
*/
	...
	IS_NPC( victim ) ?
	  get_exp_worth( victim ) < MAX_EXP_WORTH ?
	   get_exp_worth( victim ) : MAX_EXP_WORTH : victim->exp ) ;
...
void do_oinvoke( CHAR_DATA *ch, char *argument )
{
	...
	ch->name, pObjIndex->short_descr, pObjIndex->vnum,
	  pObjIndex->name, obj->level, quantity );
...
void do_showclass( CHAR_DATA *ch, char *argument )
{
    ...
    pager_printf_color( ch, "&wReconnectother: &W%s\n\r",
        class->reconnect_other?class->reconnect_other:"Default");
...
void do_setclass( CHAR_DATA *ch, char *argument )
{
	...
	for ( cl = 0; cl < MAX_CLASS && class_table[cl]; cl++ )
	{
	    if ( !class_table[cl]->who_name ) continue;
	    if ( !str_cmp(class_table[cl]->who_name, arg1) )
	    {
		class = class_table[cl];
		break;
